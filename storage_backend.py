# storage_backend.py — accés unifié CSV / Google Sheets + ETag simple
...
def compute_etag(df: pd.DataFrame, name: str) -> str:
    try:
        if df is None or df.empty:
            return "empty"
        # Minimal: ID + Updated_At si présent
...
def ensure_df_source(name: str, cols: list, paths: Dict[str, Path] = None, ws_func=None) -> pd.DataFrame:
    """Charge une table depuis Google Sheets si ws_func est fourni et storage_backend=='gsheets',
    ...
    if backend == "gsheets":
        ...
        try:
            ...
            st.error(f"Lecture Google Sheets échouée ({tab}): {e}")
...
def save_df_target(name: str, df: pd.DataFrame, paths: Optional[Dict[str, Path]] = None, ws_func=None):
    ...
    if backend == "gsheets":
        ...
        tab = SHEET_NAME.get(name, name)
        ws = ws_func(tab)
        # Verrou optimiste : comparer avec l'état actuel du remote
        try:
