...
if page == "CRM (Grille centrale)":
    ...
    # liste compacte "ID – Prénom Nom – Société"
    def _label_contact(row):
        return f"{row['ID']} — {row['Prénom']} {row['Nom']} — {row['Société']}"

    options = [] if df_contacts.empty else df_contacts.apply(_label_contact, axis=1).tolist()
    id_map = {} if df_contacts.empty else dict(zip(options, df_contacts["ID"]))

    col_top1, col_top2 = st.columns([3, 1])
    sel_label = col_top1.selectbox(
        "Contact sélectionné (sélecteur maître)",
        [""] + options,
        index=0,
        key="select_contact_label"
    )
