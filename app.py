...
if page == "Dashboard":
    ...
    with st.expander(f"➕ Ajouter {page[:-1].lower()}"):
        with st.form(f"form_{page_key}"):
            ...
            # Autres pages similaires - formulaires adaptés
            else:
                st.info(f"Formulaire {page} à implémenter selon besoins spécifiques")
                if st.form_submit_button("Enregistrer"):
                    show_alert("Fonctionnalité en développement", "error")
        ...
        try:
            ...
            with pd.ExcelWriter(output, engine='openpyxl') as writer:
                for name, schema in schemas.items():
                    pd.DataFrame(columns=list(schema.keys())).to_excel(writer, sheet_name=name.capitalize(), index=False)
            output.seek(0)
            ...
            try:
                ...
                if missing:
                    ...
                    for sheet, schema in required_sheets.items():
                        try:
                            import_df = pd.read_excel(uploaded, sheet_name=sheet)
                            missing_cols = [c for c in schema.keys() if c not in import_df.columns]
                            if missing_cols:
                                errors.append(f"{sheet}: colonnes manquantes {missing_cols}")
                            else:
                                data_to_import[sheet] = import_df
                        except Exception as e:
                            errors.append(f"Erreur {sheet}: {e}")
                    ...
                    if errors:
                        ...
                        for sheet, import_df in data_to_import.items():
                            with st.expander(f"Aperçu {sheet} ({len(import_df)} lignes)"):
                        ...
                        if st.button("🚀 Confirmer import"):
                            ...
                            try:
                                for sheet, new_df in data_to_import.items():
                                    file_key = sheet.lower()
                                    if file_key in FILES and file_key in schemas:
                                        existing_df = safe_load_df(FILES[file_key], schemas[file_key])
                                        ...
                                        safe_save_df(combined, FILES[file_key])
                                        success_count += 1
...
# Footer
st.markdown("---")
st.markdown("**IIBA Cameroun CRM** v2.0 - Développé avec ❤️ pour la Business Analysis")
